...
// Level 2 to 1st cargoship dropoff location
    ...
    AddSequential(new AutoDriveAndRotateCommand(1.0, 0.0, 0.4, 270.0, 0.2));    //rotate & move toward cargoship but slow //1.5
...
// score hatch 1
    ...
    AddSequential(new SwerveDrivetrainDriveAndRotateCommand(0.05, 0.02, 0.0, 0.0, 270.0)); //.2
    
    AddSequential(new AutoScoreHatchCommandGroup());

// drive to feeder
    AddSequential(new SwerveDrivetrainJoystickSetFieldFrame(true));
    AddSequential(new AutoDriveAndRotateCommand(2, -165, 0.4, 180.0, 1.0));
    AddSequential(new SwerveDrivetrainJoystickSetFieldFrame(false));

// pickup hatch 2
    AddSequential(new AutoAquireHatchCommandGroup(true));
    AddSequential(new SwerveDrivetrainSetOpenLoop(0.2, 0.0,-0.3,0.0));

// drive to cargo ship
    AddSequential(new SwerveDrivetrainJoystickSetFieldFrame(true));
    AddSequential(new AutoDriveAndRotateCommand(0.2, 12.0, 0.4, 270.0, 1.0));
    AddSequential(new SwerveDrivetrainDriveAndRotateCommand(0.2, 0.02, 0.0, 0.0, 270.0));
    AddSequential(new AutoDriveAndRotateCommand(1.0, 0.0, 0.4, 270.0, 1.0));
    AddSequential(new AutoDriveAndRotateCommand(0.5, 0.0, 0.4, 270.0, 0.2));
